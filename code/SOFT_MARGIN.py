...
X_temp = iris.data[:, :2]  # we only take the first two features. We could avoid this ugly slicing by using a two-dim dataset
y_temp = iris.target
...
response3 = raw_input("Please enter type of SVM margin : ")
...
for train_index, test_index in kf:
    ...
    alpha_value = findQPSolver(X_Train_Data, Y_Train_Data, C=1000)
    W_Value = Find_W_Value(X_Train_Matrix,Y_Train_Matrix,alpha_value, C=1000)
    W0_Value = Find_W0_Value(X_Train_Matrix,Y_Train_Matrix,W_Value,alpha_value, C=1000)
    print W_Value
    print W0_Value
    for i in range(len(alpha_value)):
        if (0<alpha_value[i]<0.1):
    ...
    predictedy = discriminant_function(W_Value,W0_Value,X_Test_Matrix)
